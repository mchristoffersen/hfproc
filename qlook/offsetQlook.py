...
def findOffsetPC(rx0, refchirp, cf, fs):
  ...
  rx0 = np.roll(rx0, len(mt)//2, axis=0)
  rx0 = pulseCompress(rx0, refchirp)
  argmx = np.argmax(np.abs(rx0), axis=0)

  return (mode(argmx).mode[0] - len(mt)//2)
...
def genQlook(fname, outd):
  ...
  if(sig == b"impulse"):
    ...
    shift = findOffsetPC(rx0, refchirp, cf, fs)
  ...
  if(rx0.shape[1] < 20):
    exit()
  saveImageOffset(rx0, fs, outd + '/' + fname.split('/')[-1].replace(".h5", "_shiftDT.png"), shift)
  f.close()
