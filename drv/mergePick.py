...
def main():
  ...
  parser = argparse.ArgumentParser(
    description="Program for merging picks info to hdf5 files"
  )
  parser.add_argument("srf", help="Pick files", nargs="+")
  ...
  for data in data:
    pickf = data.replace("/hdf5/", "/pk/").replace(".h5", "_pk.csv") #.replace("IRARES1B_","").replace("IRUAFHF1B_","")

    if(not os.path.isfile(pickf)):
      continue

    pick = np.loadtxt(pickf)
